...
\begin{algorithmic}
\State $\bm{x}_k \coloneqq \bm{x}_0, \epsilon > 0$
...
\begin{algorithmic}
\State $\bm{x}_k, \bm{x}_{k-1} \coloneqq \bm{x}_0, \bm{H}_k \coloneqq \bm{H}_0 $, $\bm{H}_0$ symmetric positive definite, $\epsilon > 0$
\While{$\norm{\nabla f(\bm{x}_k)} > \epsilon$}
...
is required in general. An overview over the entire L-BFGS algorithm is given in \cref{alg:lbfgs}, where the details of maintaining the history
of the at most $m$ most recent $\bm{x}_k, \bm{s}_k, \bm{y}_k, \rho_k$ are omitted for the sake of clarity.

\begin{algorithm}
\caption{L-BFGS method}\label{alg:lbfgs}
\begin{algorithmic}
\State $\bm{x}_k \coloneqq \bm{x}_0, \bm{H}_k \coloneqq \bm{H}_0 $, $\bm{H}_0$ symmetric positive definite, $\epsilon > 0$
\While{$\norm{\nabla f(\bm{x}_k)} > \epsilon$}
\State $\bm{s_k} = \bm{x}_k - \bm{x}_{k-1}, \bm{y}_k = \nabla f(\bm{x}_k) - \nabla f(\bm{x}_{k-1}), \rho_k = 1 / (\bm{s}^T_k \bm{y}_k)$
\State compute $\bm{p}_k$ via the two-loop recursion algorithm (\cref{alg:lbfgs-recursion})
\State $\alpha_k = 1$
\If{$\alpha_k$ does not satisfy the strong Wolfe conditions}
\State compute $\alpha_k$ that satisfies the strong Wolfe conditions
\EndIf
...
As discussed in \cref{sss:quasi-newton} and \cref{sss:limited-memory-quasi-newton}, 
the choice of the initial inverse Hessian approximation $\bm{H}_0$ has a strong impact on the performance of Quasi-Newton
methods such as the BFGS and L-BFGS method. Popular choices such as scaled versions of the identitiy matrix generally do not
satisfy any formal optimality conditions but are picked heuristically instead. This suggests that finding more suitable 
candidates for $\bm{H}_0$ -- or alternatively for $\bm{B}_0$ -- is an avenue towards improving the convergenve properties of 
Quasi-Newton methods. It stands to reason that due to its effectiveness in PD, $\bm{S}$ is a viable choice for the 
initial Hessian approximation $\bm{B}_0$ for the minimization of \cref{eq:pd-minimization-q2} \cite{liu2017}. 
...
Liu et al.\ focus on the L-BFGS method with $\bm{B}_0 = \bm{S}$ \cite{liu2017}.
