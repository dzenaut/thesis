...
\section{Results}
\textcolor{red}{What is the general goal of the experiments?}
...
\paragraph{Analyzing Solver Convergence.}
...
\subsection{Experiments}\label{ss:experiments}
...
effect is caused by oscillations of the Gauss-Seidel solver at the core of XPBD, since it does not appear for solvers that have a global optimization step.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textcolor{red}{Some glue text here!} During the untwisting beam experiments using the strain material model discussed above, each solver reduces 
...
In our discussion of \autoref{fig:strain-beam-untwist-objectives-split}, we hypothesized that the reason for the comparably large constraint term 
observed after 1000 XPBD iterations is due to oscillations of the Gauss-Seidel-type solver at the core of XPBD. Since the severity of the oscillations 
is expeced to increase with both time step size and stiffness values, the data in \autoref{fig:strain-beam-twist-xpbd-failures} supports this claim.
However, the time step and the stiffness alone do not explain why XPBD does not fail during any of the unwisting beam experiments, even when the same 
time step and stiffness are used. One possible explanation is that the position constraints and the tetrahedral 
strain constraints in the twisting beam example are fundamentally incompatible in the following sense: There are no particle positions $\vecm{q}$ for 
which both the position constraints and the strain constraints are satisfied at the same time. On the other hand, all constraints are satisfied 
in the reference configuration of the beam in the untwisting beam scenario. It is natural to assume that the former case is more challenging for 
Gauss-Seidel-type solvers.
...
\autoref{fig:twisting-beam-xpbd-failure-geometry}.
...
% TODO:
% - Proof read stuff from yesterday
