...
as backtracking and is outlined in \cref{alg:backtracking}.

\begin{algorithm}
\caption{Backtracking}\label{alg:backtracking}
\begin{algorithmic}
\State \textbf{require } $\tilde{\alpha} > 0, c, \rho \in (0, 1), 0 < t < \tilde{\alpha}$
\Procedure{backtrack}{$\bm{x}_k$, $\bm{p}_k$, $\tilde{\alpha}$, $\rho$, $t$}
\State $\alpha = \tilde{\alpha}$
\While{$f(\bm{x}_k + \alpha \bm{p}_k) \leq f(\bm{x}_k) + c \alpha \nabla f(\bm{x}_k)^T \bm{p}_k$ and $\alpha > t$}
...
\cref{alg:pd-lbfgs}. Note that the search direction at iteration $k$ is denoted $\bm{r}_k$ instead of $\bm{p}_k$ in order to avoid
confusion with the projection point of the $k$-th constraint in the PD framework. The occurrences of $f$ 
in the two-loop recursion (\cref{alg:lbfgs-recursion}) need to be replaced by $g$ in the appropriate 
places. Again, the details of maintaining the history of $\bm{s}, \bm{y}, \rho$ are omitted for the sake of clarity.
