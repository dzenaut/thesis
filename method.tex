...
\paragraph{Explicit Euler Integration.}
...
\paragraph{Symplectic Euler Integration.}
...
For general nonlinear materials, computing the integral in \autoref{eq:elastic-energy} is infeasible. Thus, simplifying assumptions are required 
...
The insight that PD is a special case of Quasi-Newton methods applied to $\autoref{eq:pd-minimization-q}$ can be leveraged to
...
at the current iterate at all. Combining both approaches by using $\matm{S}$ as the initial Hessian approximation in the LBFGS-method 
to accelerate convergence is discussed in \cref{ss:qn-pd}. In the rest of this thesis, we refer to the resulting solver as the QN solver.
...
optimization problem and attempt to minimize the objective function of the variational form of implicit Euler integration (see \cref{ss:variational-implicit-euler}). 
...
\autoref{ss:experimental-setup} and discuss how insights on various solver properties can be gained from the positions achieved after each iteration in 
\autoref{ss:analysis-solvers}. During our comparison of the solvers, we exclude PBD due to the fact that it is not derived from the equations of motion and does not produce 
physically accurate results (see \cref{ss:pbd-properties}). We provide experimental data to justify this decision in \autoref{ss:physical-pbd}. Additionally, before a meaningful 
comparison of ADMM with other solvers is possible, suitable ADMM weights need to be determined (see \cref{s:admm}). We provide insights on choosing ADMM weights in \autoref{ss:admm-weights}. 
We start our comparison between XPBD and PD-style solvers on simulations using only tetrahedral constraints for the strain material model (see \cref{ss:material-models}) 
in \autoref{ss:untwisting-beam-strain}. The effects of adding position constraints that are incompatible with the material constraints are analyzed in 
\autoref{ss:twisting-beam-strain}. Finally, the experiments in \autoref{ss:untwisting-beam-strain} are repeated for the neohookean material model (see \cref{ss:material-models}) 
in \autoref{ss:untwisting-beam-neohookean} to gain insights on the compatibility of the solvers with more complicated non-linear material models.

\subsection{Experimental Setup}\label{ss:experimental-setup}
Experiments start from the deformed configuration of a cuboid beam that has been twisted in opposite directions at both ends (\cref{fig:twisting-beam-setup}, left).
...
model from artifacts caused by incompatible constraints. A representative set of experiments is discussed in \autoref{ss:untwisting-beam-neohookean}.
...
As discussed in \autoref{ss:variational-implicit-euler}, the constraint term of the objective function corresponds to the sum of the constraint energies, while the 
...
experiments described in \autoref{ss:experimental-setup}, the inertial term can also be interpreted as the kinetic energy of the beam. To see this, note that both the initial 
...
damping which is expected to reduce the energy of the beam over time (see \autoref{ss:numerical-integration}).
...
constraint projection (see \cref{ss:xpbd-properties}). Additionally, in the context of real-time applications, the number of solver iterations is often fixed to ensure 
...
simulated using PBD depends on the number of iterations and the chosen time step (see \cref{ss:pbd-properties}). Accordingly, in the limit of infinite iterations and zero 
...
the objective function of the beam (see \cref{ss:analysis-solvers}). We focus on untwisting beam experiments using strain constraints. For 
...
of \SI{1e-2}{\second} is used (\cref{fig:strain-weights-admm} a), the ADMM solver converges to the same objective for all of the weights, except for the 
...
material stiffness. To save time, we reuse the ADMM weights determined for the untwisting beam experiments (see \cref{ss:untwisting-beam-strain}) in the twisting 
beam experiments (see \cref{ss:twisting-beam-strain}). However, it is not clear whether ADMM weights can be reused across different simulation scenarios, even if the time step 
...
\begin{figure}[h]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \subcaption{Time step \SI{1e-3}{\second}}
    \end{subfigure}
    \caption{Values of the objective function of the variational form of implicit Euler integration (\cref{eq:variational-implicit}) over the iterations of a single time 
...
transform (see \cref{ss:material-models}). Thus, we use them as a starting point for highlighting differences between the different solvers.
...
iteration number in \autoref{fig:strain-beam-untwist-objectives}. Note that the results are representative for a large range of time step sizes and stiffness 
...
achieved by any of the solvers. The relative errors are shown in \autoref{fig:rel-errors}. Note that the relative error of ADMM is only 
...
\begin{figure}[h]
    ...
    \caption{Values of the constraint (top) and inertial (bottom) terms of the objective function of the variational form of implicit Euler integration 
        (\cref{eq:variational-implicit}) over the iterations of single time step of size \SI{1e-2}{\second} for an untwisting beam with strain 
...
the surface of the XPBD geometry is smooth. In the discussion of \autoref{fig:strain-beam-untwist-geometries} b and \autoref{fig:twisting-beam-xpbd-failure-geometry},
...
energy of the beam (see \cref{ss:analysis-solvers}), the results in \autoref{fig:strain-beam-untwist-objectives-split} suggest that some of the 
...
objective function to the minimum achieved by the PD-style solvers yet (see \cref{fig:strain-beam-untwist-objectives}). After 10 iterations, the QN solver has reduced 
...
Closer inspection of \autoref{fig:strain-beam-untwist-geometries} b shows that the XPBD geometry is noticably rugged compared to the geometries of the PD-type 
solvers, which 
...
is larger than the initial constraint objective in \autoref{fig:strain-beam-untwist-objectives-split}. While it is difficult to verify, it is natural to assume that this 
...
fact that the XPBD geometry does appear smooth after 1000 iterations (see \cref{fig:strain-beam-untwist-geometries}) would suggest that oscillations are particularly 
...
iteration durations in \autoref{fig:strain-beam-untwist-objectives-time}. However, the provided iteration durations should be treated as a rough indication of the 
...
take roughly 4 seconds each. Due to the similar computational costs per iteration, the conclusions that can be drawn from \autoref{fig:strain-beam-untwist-objectives-time}
...
\begin{figure}[h]
    ...
    \caption{Values of the objective function of the variational form of implicit Euler integration (\cref{eq:variational-implicit}) over the accumulated durations of 
...
inertial and constraint terms. This is analogous to the experiments conducted in \autoref{fig:strain-beam-untwist-objectives-split} to investigate 
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Values of the constraint (top) and inertial (bottom) terms of the objective function of the variational form of implicit Euler integration 
        (\cref{eq:variational-implicit}) over the iterations of a single time step of size \SI{1e-1}{\second} for an untwisting beam using strain constraints with 
    stiffness \num{1e9}.}
    \label{fig:strain-beam-untwist-objectives-split-large-ts}
\end{figure}

Again, \autoref{fig:strain-beam-untwist-objectives-split-large-ts} shows that the solvers mostly decrease the constraint objective and increase the inertial term during each 
...
iterations. \autoref{fig:strain-beam-untwist-geometries-large-ts} a shows a comparison between the XPBD and QN geometries. Similar to the 
...
\autoref{fig:strain-beam-untwist-objectives-split-large-ts} does not go along with a large decrease in the constraint term: The spike is caused by 
rigid-body motion, to which elastic energy potentials are invariant. Secondly, we discussed in \autoref{ss:pd-quasi-newton} that PD can be considered 
...
\begin{figure}[h]
    ...
    $\vecm{v}_k$ at iteration $k$ are computed via Verlet integration (see \cref{ss:analysis-solvers}).
}
...
\autoref{fig:strain-beam-twist-typical-objectives}. While the results obtained for the QN and PD solvers are qualitatively the same for all time step 
...
\autoref{fig:strain-beam-twist-typical-objectives} show that both QN and PD converge to a configuration with a lower objective function value than the initial 
...
(see \cref{fig:strain-beam-untwist-objectives-large-ts}). This is particularly noticable for the PD solver. \autoref{fig:strain-beam-twist-typical-objectives} 
...
significantly larger than the initial objective in \autoref{fig:strain-beam-twist-typical-objectives}. Again, this observation is repeated for a variety of 
...
\begin{figure}[h]
    ...
    \caption{Values of the objective function of the variational form of implicit Euler integration (\cref{eq:variational-implicit}) over the iterations of 
...
During the discussion of the untwisting beam experiments using large time steps and stiffness values (see \autoref{fig:strain-beam-untwist-objectives-large-ts}),
...
undeformed reference configuration over a single time step in \autoref{fig:strain-beam-untwist-geometries-large-ts} b. In contrast, the beam is only twisted by a 
...
and stiffness value. As indicated by the data in  \autoref{fig:strain-beam-twist-typical-objectives}, this is not always the case for the twisting beam experiments. 
...
\autoref{fig:strain-beam-twist-xpbd-failures}. Note that the results are identical when restricting to the constraint term of the objective function. 
...
term to increase. \autoref{fig:strain-beam-twist-xpbd-failures} shows that XPBD fails in settings where large stiffness 
...
data presented in \autoref{fig:strain-beam-twist-xpbd-failures}.
...
\autoref{fig:twisting-beam-xpbd-failure-geometry}. The left panel shows that the surface of the XPBD geometry is noticably rugged. Recall that 
the same observation is made after 10 XPBD iterations in the untwisting beam setting (see \cref{fig:strain-beam-untwist-geometries}). The 
...
we zoom in on the geometry at one end of the beam in the right panel of \autoref{fig:twisting-beam-xpbd-failure-geometry}. The geometry that the QN solver 
...
between the incompatible position constraints and local strain constraints. Together, our results from \autoref{fig:strain-beam-untwist-geometries} and 
\autoref{fig:twisting-beam-xpbd-failure-geometry} suggest that oscillations can cause noticable geometric artifacts if an insufficiently low number of 
...
positions (see \cref{ss:xpbd-properties}). Thus, XPBD has more freedom to update particle positions in such a way that the position constraints 
...
large stiffness value of \num{1e12} used for the position constraints in \autoref{fig:twisting-beam-xpbd-failure-geometry} is insufficient for moving particles 
...
should be reflected in a larger inertial term compared to the QN solver. In \autoref{fig:strain-beam-twist-xpbd-large-inertial-terms}, we list combinations 
...
The results are in line with the observations from the previous experiments. In particular, \autoref{fig:strain-beam-twist-xpbd-large-inertial-terms} shows 
that the inertial term achieved by XPBD is larger than the inertial term achieved by QN for the parameters used in 
\autoref{fig:twisting-beam-xpbd-failure-geometry} (time step \SI{1e-2}{\second}, stiffness \num{1e12}). The observation that the inertial term achieved by 
...
In our discussion of \autoref{fig:strain-beam-untwist-objectives-split}, we mentioned that the inertial term achieved by 
...
\SI{1e-2}{\second} and stiffness \num{1e10} in \autoref{fig:strain-beam-twist-objectives}. After increasing the objective function value during the first 40 
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Values of the constraint (top) and inertial (bottom) terms of the objective function of the variational form of implicit Euler integration 
        (\cref{eq:variational-implicit}) over the iterations of a single time step of size \SI{1e-1}{\second} for an untwisting beam with strain 
    constraints with stiffness \num{1e9}.}
    \label{fig:strain-beam-twist-objectives-split}
\end{figure}

Again, we create separate plots for the constraint and inertial terms of the objective function in \autoref{fig:strain-beam-twist-objectives-split}. The graph for the 
constraint term looks almost identical to the graph for the entire objective function in \autoref{fig:strain-beam-twist-objectives}. During the first 40 iterations, the 
...
significanty lower than the one achieved by the other PD-style solvers. The most dramatic decrease in the constraint term occurs between 40 and 60 ADMM iterations. At the 
same time, ADMM's inertial term increases dramatically. The final inertial term achieved by ADMM is significanty higher than the inertial terms achieved by all other solvers, 
including XPBD.
...
hypothesis is supported by the observation that ADMM increases the objective during the first 20 iterations (see \cref{fig:strain-beam-twist-objectives}). Informally, this 
...
To gain more insights, we take a closer look at the ADMM geometries after 20 and 1000 iterations in \autoref{fig:strain-beam-twist-admm-geometries}. After 20 ADMM iterations, 
...
The results in \autoref{fig:strain-beam-twist-admm-geometries} support the hypothesis that ADMM converges to a global minimum of the objective function, whereas QN and PD 
...
interpreted as the energy of the beam at a configuration (see \cref{ss:analysis-solvers}). Then, it is obvious that the overtwisting motion required to achieve this 
...
energies (see \autoref{ss:admm-weights}). Finally, \autoref{fig:strain-beam-twist-objectives} and \autoref{fig:strain-beam-twist-admm-geometries} highlight the ramifications
...
\autoref{fig:strain-beam-twist-admm-artificial}. For the large time step \SI{1e-1}{\second}, the ADMM solver converges to the global minimum for stiffness values as 
...
(see \cref{ss:variational-implicit-euler}). Still, it is likely that the choice of the ADMM weights has a strong impact on whether the ADMM solver converges to the 
...
\autoref{fig:strain-beam-twist-objectives-admm-failure}. The graphs show that ADMM oscillates between configurations with objective function values that are larger 
...
Together with the results discussed above, the data from \autoref{fig:strain-beam-twist-objectives-admm-failure} suggests that ADMM drives particle 
...
this failure to converge occurs for the parameters listed above, but not for the parameters in \autoref{fig:strain-beam-twist-admm-artificial}. Again, the choice 
...
parameter screens for the untwisting beam experiments (see \cref{ss:admm-weights}). In light of the similarities between PD and ADMM with weights $w_i = \sqrt{k}$, 
