...
The PD solver can be derived by plugging the distance functions from \autoref{eq:pd-distance} into the variational form of implicit
...
\begin{equation}\label{eq:pd-minimization}
    ...
    - \vecm{p}_j}^2_F + \delta_{\cman{C}^\prime_j}(\vecm{p}_j)
\end{equation}
...
\autoref{eq:pd-minimization} over the auxiliary variables $\vecm{p}_j$ while keeping the positions $\vecm{q}$ fixed. For each 
...
\noindent In the global step, the minimization problem \autoref{eq:pd-minimization} is optimized over the positions $\vecm{q}$ 
while keeping the auxiliary variables $\vecm{p}_j$ fixed. The optimization problem for the global solve is 
given by
