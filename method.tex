...
\subsubsection{PBD Constraint Solver}\label{sss:pbd-constraint-solver}
...
\begin{equation}\label{eq:pbd-distance}
    C_{\text{dist}}(\bm{p}_{i_1}, \bm{p}_{i_2}) = \vert \bm{p}_{i_1} - \bm{p}_{i_2} \vert.
\end{equation}
...
\For{all constraints $C_j$ with cardinality $n_j$, particle indices $i_1, \ldots, i_{n_j}$}
\If{$C_j$ is an equality constraint or $C_j(\bm{p}_{i_1}, \ldots, \bm{p}_{i_{n_j}}) < 0$}
\For{all particles $i \in \{ i_1, \ldots, i_{n_j} \}$}
\State $\Delta \bm{p}_i = -\frac{C_j(\bm{p})}{\sum_{i \in \{ i_1, \ldots, i_{n_j} \}} w_i \vert \nabla_{\bm{p}_i}C(\bm{p}) \vert^2 } w_i \nabla_{\bm{p}_i} C_j(\bm{p})$
\State $\bm{p}_i = k \Delta \bm{p}_i$ or $\bm{p}_i = (1-(1-k)^{1/n_s}) \Delta \bm{p}_i$
\EndFor
\EndIf
\EndFor
\EndFor
...
\subsubsection{Properties of PBD}\label{sss:pbd-properties}
...
in lines 6-7 of \cref{alg:pbd-solver}
...
While the simulated object's global linear and angular momentums are preserved, the linear momentums of individual particles are at risk of 
