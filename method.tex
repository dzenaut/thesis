...
\begin{equation}\label{eq:variational-implicit}
    \min_{\vecm{q}_{n+1}} \frac{1}{2h^2} \norm{\vecm{q}_{n+1} - \tilde{\vecm{q}}}^2_F + \sum_j \psi_j(\vecm{q}_{n+1}).
\end{equation}
...
building block like a matrix factorization.

Without knowing all the details I would say that something between a half and full page should suffice on LBFGS (with references to 
...
Let the positions of $m$ particles in a mesh be stored in a matrix $\vecm{q} \in \mathbb{R}^{m \times 3}$ with deformation gradient $\vecm{f} 
\coloneqq \matm{F(\vecm{q})} \in \mathbb{R}^{3 \times 3}$. Then, energy potentials of the general form $\psi(\matm{E}(\vecm{f}))$, where 
$\matm{E}(\vecm{f})$ is a strain measure that depends on the deformation gradient of a discrete element, are frequently used in nonlinear 
continuum mechanics \cite{sifakis2012}. If $\matm{E}$ is Green's strain measure $\matm{E_{\text{Green}}}$ defined by 

\[
\matm{E_{\text{Green}}}(\vecm{f}) = \frac{1}{2}(\matm{F^TF} - \matm{I})
\]

\noindent then $\matm{E_{\text{Green}}}(\vecm{f}) = \matm{0}$ is equivalent to $\matm{F^TF} = \matm{I}$. Thus, $\matm{E_{\text{Green}}}(\vecm{f}) = 0$ 
defines a constraint manifold of configuarion whose deformation gradients $\vecm{f}$ are rotation matrices. These deformations are exactly 
the rigid-body 
transforms, i.e. transforms that alter the body's position and orientation but keep the body's volume undeformed. Assuming that $\psi(\matm{0}) 
= \rho$ for some $\rho \in \mathbb{R}$ and that $\psi$ reaches its minimum at the undeformed configuration, then 

\[
    d(\matm{E_{\text{Green}}}(\vecm{f})) = \psi(\matm{E_{\text{Green}}}(\vecm{f})) - \rho
\]
...
manifold $\cman{C}$ -- which 
can be different from $\matm{E_{\text{Green}}(\vecm{f})} = 0$ -- and a distance measure $d$ of the body's current configuration to that constraint 
manifold. Formally, this leads to energy potentials of the 
following form:

\[
    \psi({\vecm{q}}) = \min_{\vecm{p}}(d(\vecm{q}, \vecm{p}) + \delta_{\cman{C}}(\vecm{p})).
\]

\noindent Here, $\vecm{p} \in \mathbb{R}^{r \times 3}, r \in \mathbb{N}$ are auxiliary projection variables \textcolor{red}{(Fabian points
out that it is quite surprising that $p$ and $q$ do not have same dimensions, i.e. that there is no particle to particle correspondence.
This is because I use the notationa from \cite{liu2017} to stay consistent. It is better to use the original notation from \cite{bouaziz2014}
when introducing PD for the sake of clarity. Later write a paragraph to show that both formulations are equivalent and use the one from 
\cite{liu2017})} and $\delta_{\cman{C}}(\vecm{p})$ is an indicator function with 

\[
    \delta_{\cman{C}}(\vecm{p})= 
\begin{cases}
0,& \text{if } \vecm{p} \text{ lies on the constraint manifold } \cman{C}\\
\infty,& \text{otherwise.}
\end{cases}
\]

\noindent Define $\vecm{p_{\vecm{q}}}$ such that $\psi({\vecm{q}}) = d(\vecm{q}, \vecm{p_{\vecm{q}}}) + \delta_{\cman{C}}(\vecm{p_{\vecm{q}}})$ \textcolor{red}{(Fabian
prefers argmin notation here.)}. Then obviously 
$\delta_{\cman{C}}(\vecm{p_{\vecm{q}}}) = 0$, meaning that $\vecm{p_{\vecm{q}}}$ lies on $\cman{C}$. Together, $\vecm{p_{\vecm{q}}}$ is the 
configuration on the constraint manifold $\cman{C}$ with minimal distance $d(\vecm{q}, \vecm{p_{\vecm{q}}})$ to current configuration $\vecm{q}$.
Consequently, $\psi(\vecm{q})$ measures the distance of $\vecm{q}$ to the constraint manifold $\cman{C}$.
...
\begin{equation}\label{eq:pd-distance-measures}
    d(\vecm{q}) = \frac{w}{2} \norm{\vecm{gq} - \vecm{p}}^2_F,
\end{equation}

\noindent where $\matm{G} \in \mathbb{R}^{r \times m}$ for some $r \in \mathbb{N}$ and $w$ is the constraint stiffness. In summary, PD 
energy potentials have the following form:

\begin{equation}\label{eq:pd-potentials}
    \psi({\vecm{q}}) = \min_{\vecm{p}} \frac{w}{2} \norm{\vecm{gq} - \vecm{p}}^2_F + \delta_{\cman{C}}(\vecm{p}).
\end{equation}
...
can be approximated in terms of PD energy potentials (\cref{eq:pd-potentials}) by setting $\matm{G} \in \mathbb{R}^{3 \times m}$ to the 
matrix $\vecm{a}$ that maps $\vecm{q}$ to the transpose of the deformation gradient of the tetrahedron $\vecm{f}^T$ and $\cman{C}$ to the 
set of rotational matrices SO(3) \textcolor{red}{(Don't forget to change this for the \cite{bouaziz2014} notation. In this setting, p
is the set of particle positions for which the deformation gradient is a rotation.)}. Typically, the weight $w = kV$, where $V$ is 
the volume of the undeformed tetrahedron and $k$ is a 
...
\psi({\vecm{q}}) = \min_{\vecm{p}} \frac{w}{2} \norm{\matm{A}\vecm{q} - \vecm{p}}^2_F + \delta_{\text{SO}(3)}(\vecm{p})
    = \min_{\vecm{p}} \frac{w}{2} \norm{\vecm{f}^T - \vecm{p}}^2_F + \delta_{\text{SO}(3)}(\vecm{p}).
\end{equation}

\noindent It is easy to show that this energy is zero if and only if $\vecm{f}$ itself is a rotational matrix and grows as the 
...
The last property follows from the fact that the distance measure $d$ in \autoref{eq:pd-distance-measures} has no dependencies between 
