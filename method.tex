...
Note that since $\bm{q} \in \mathbb{R}^{m \times 3}$, the distance measure $d$ has no dependencies between $x$-, $y$- and 
...
\subsubsection*{Projective Implicit Euler Solver}
We start by substituting the PD energy potentials into the variatonal form of implicit Euler integration, which yields the following joint 
optimization problem over the positions $\bm{q}_{n+1}$ and auxiliary variables $\bm{p}_i$

\[
    \min_{\bm{q}, \bm{p_i}} \frac{1}{2h^2} \norm{\bm{M}^{1/2}(\bm{q} - \bm{s}_n)}^2_F + \sum_i \norm{\bm{G}_i\bm{q} - \bm{p_i}}^2_F + 
    \delta_{\bm{C}_i}(\bm{p_i}).
\]

We write $\bm{q} \coloneqq \bm{q}_{n+1}$ for the sake of brevity. 

This optimization problem is optimized using a local/global alternating minimization technique. Local and global steps are carried out
sequentially for a fixed number of iterations during each timestep. The local step consists of minimizing the objective function over 
...
In the global step, the minimization problem is optimized over the positions $\bm{q}$ while keeping the auxiliary variables $\bm{p}_i$ fixed.
...
By design of the PD energy potentials, the objective function of the global optimization problem is quadratic in the positions $\bm{q}$. 
Consequently, the minimization can be carried out in a single step by picking $\bm{q}$ such that the first-order optimality conditions are
satisfied \cite{nocedal2006}. This requires solving the linear system

\[
    (\frac{\bm{M}}{h^2} + \sum_i w_i \bm{G}_i^T \bm{G}_i)\bm{q} = \frac{\bm{M}}{h^2}\bm{s}_n + \sum_i w_i \bm{G}_i^T \bm{p}_i.
\]

Since the system matrix is constant as long as the constraints remain unchanged it can be prefactorized at initialization, making efficient
...
algorithm is given \textcolor{red}{below}.

\begin{algorithm}
\caption{Projective Implicit Euler Solver}
\begin{algorithmic}
...
Here, $f$ is called the objective function. Most algorithms are incapable of finding global minimizers of general nonlinear functions. 
...
In most cases, it is infeasible to compute $\alpha_k$ exactly. Instead, the idea is to compute an approximation of $\alpha_k$ such that 
...
\begin{align*}
    f(\bm{x}_k + \alpha_k \bm{p}_k) &\leq f(\bm{x}_k) + c_1 \alpha_k \nabla f(\bm{x}_k) \bm{p}_k \\
    \abs{\nabla f(\bm{x}_k + \alpha_k \bm{p}_k)^T \bm{p}_k} &\leq c_2 \abs{\nabla f(\bm{x}_k)^T \bm{p}_k}
\end{align*}

for some constants $c_1 \in (0, 1), c_2 \in (c_1, 1)$. The first condition is called the sufficient decrease or the Armijo condition 
and states that the reduction in $f$ should be proportional to both the step length $\alpha_k$ and the directional derivative 
$\nabla f(\bm{x}_k \bm{p}_k)$. Informally, the second condition, known as the curvature condition, ensures that there is no more fast 
...
This method is called steepest descent. While simple, steepest descent exhibits poor performance, even for simple problems \cite{nocedal2006}. 
...
The model function $m_k$ has a unique minimizer if $\nabla^2 f(\bm{x}_k)$ is positive definite. In this case, the Newton direction is defined
...
The better the quadratic model function $m_k(\bm{p})$ approximates $f(\bm{x}_k + \bm{p})$ around $\bm{x}_k$, the more reliable is the 
Newton direction. 
...
needs to be solved at every iteration as the Hessian matrix changes with the current iterate $\bm{x}_k$. For these reasons, while a single
...
Due to the shortcomings of Newton's method mentioned \textcolor{red}{above}, it can be favorable to simply approximate the Newton direction
...
computed without the need to compute the Hessian $\nabla^2 f(\bm{x}_k)$ during each iteration \cite{nocedal2006}. Usually, multiple Quasi-Newton 
...
where $\bm{B}_k \in \mathbb{R}^{n \times n}$ is positive definite \cite{nocedal2006}. Note that the Newton direction is a special case of 
\textcolor{red}{the above} with $\bm{B}_k = \nabla^2 f(\bm{x}_k)$. Just like for the Newton direction $\bm{p}^N_k$, a model function $m_k$ 
...
As $\bm{B}_k \neq \nabla^2 f(\bm{x}_k)$, $m_k$ does not correspond to a second-order Taylor approximation of $f$ around $\bm{x}_k$ anymore.
Instead, $\bm{B}_k$ is picked such that the gradient of $m_k$ matches the gradient of $f$ at the last two iterates $\bm{x}_k$ and $\bm{x}_{k-1}$.
Since $\nabla m_{k}(\bm{0}) = \nabla f(\bm{x}_k + \bm{0}) = \nabla f(\bm{x}_k)$, the first condition is true independent of $\bm{B}_k$. The 
second condition yields

\[
    \nabla m_{k}(-\alpha_{k-1}\bm{p}_{k-1}) = \nabla f(\bm{x}_k) - \alpha_{k-1}\bm{B}_{k}\bm{p}_{k-1} = \nabla f(\bm{x}_k - \alpha_{k-1}\bm{p}_{k-1})
    = \nabla f(\bm{x}_{k-1}).
\]
...
where $\bm{s}_{k-1} = \bm{x}_k - \bm{x}_{k-1} = \alpha_{k-1}\bm{p}_{k-1}$. This is called the secant equation. Multiplying both sides from the 
left with $\bm{s}_{k-1}^T$ yields the curvature condition given by

\[
    \bm{s}_{k-1}^T \bm{y}_{k-1} > 0,
\]

since $\bm{B}_k$ is positive definite. Note that the curvature condition is not satisfied for arbitrary $\bm{x}_k, \bm{x}_{k-1}$ if $f$ is 
...
where $\rho_{k-1} = 1 / (\bm{s}^T_{k-1}\bm{y}_{k-1})$. Alternatively, the update formula can be given in terms of $\bm{B}_k$ as

\[
    \bm{B}_{k} = \bm{B}_{k-1} - \frac{\bm{B}_{k-1} \bm{s}_{k-1} \bm{s}^T_{k-1} \bm{B}_{k-q}}{\bm{s}^T_{k-1} \bm{B}_{k-1} \bm{s}_{k-1}}
    + \frac{\bm{y}_{k-1} \bm{y}^T_{k-1}}{\bm{y}_{k-1}^T \bm{s}_{k-1}}.
\]

Generally, using the formulation in terms of the inverse matrices $\bm{H}_k$ is preferrable since the computation of the new descent direction can
...
under mild conditions. Typically, the BFGS method dramatically outperforms steepest descent and performs comparably to Newton's method on many 
practical problems. 
