...
in practice during the simulation of deformable 3-dimensional bodies. Starting from experiments using a simple non-linear material model, we extend our analysis to more 
complicated physically-based material models and incompatible constraint sets. Here, we say that a set of constraints is incompatible if there are no particle positions 
...
Position constraints are modelled 
via springs with rest-length zero and finite stiffness for all solvers. In other words, only soft position constraints 
...
model from artefacts caused by incompatible constraints. A representative set of experiments is discussed in \autoref{ss:untwisting-beam-neohookean}.
...
required to reliably evaluate solver performance across a variety of different time step sizes and stiffnesses. In this section, we describe metrics that 
enable analyzing the convergence properties of solvers and their ability to preserve important physical invariants such as the global linear and angular momentum.
...
by the implicit positions is achieved. By comparing the objective function values for the positions after the final iteration, the quality of the configurations different
...
\paragraph{Preservation of Physical Invariants.}
...
\subsection{Physically-Baded PBD}\label{ss:physical-pbd}
...
We demonstrate this effect by example of a single solve over time steps \SI{1e-2}{\second} and \SI{1e-3}{\second} in the 
...
The untwisting beam experiments using the strain material model are suitable for analyzing solver properties in a complex, yet comparably unchallenging setting:
Challenges due to incompatible constraint sets are avoided by construction of the untwisting beam scenario and the tetrahedral strain constraints are easy to minimize 
due to the fact that corresponding restorative elastic forces are proportional to the distance between the deformation and its closest rigid-body transform (see 
\cref{ss:material-models}). Thus, we use them as a starting point for highlightin differences between the different solvers.

\paragraph{Convergence Properties.}
We start by analyzing the convergence properties of different solvers with time step size \SI{1e2}{\second} and stiffness \num{1e8} by plotting the objective function values 
over the iteration number in \autoref{fig:strain-beam-untwist-objectives}. Note that the results are representative for a large range of time step sizes and stiffness 
values. The graph shows that each of the solvers indeed mostly decreases the objective function from iteration to iteration until it converges towards a final configuration 
after a sufficient number of iterations. ADMM, PD and QN converge to configurations that yield roughly the same objective function value whereas the value obtained after 1000 
...
configuration, this is in line with the observation that QN is more successful than XPBD at reducing the constraint energy of the objective function. We point out that 
...
\begin{figure}
    ...
    \caption{Geometries of an untwisting beam with strain constraints (stiffness \num{1e8}) after 10 and 1000 iterations of a single solve over a time step 
        of \SI{1e-2}{\second} using XPBD (pink), PD (orange), ADMM (green) and QN (cyan) solvers.}
...
\paragraph{Convergence Properties of PD Using Large Time Steps and Stiffnesses.}
...
\paragraph{Visual Artefacts of PD Using Large Time Steps and Stiffnesses.}
...
\begin{figure}
    ...
    \caption{Geometries of an untwisting beam with strain constraints (stiffness \num{1e9}) after 1000 iterations of a single solve over a time step 
        of \SI{1e-1}{\second} using XPBD (pink), PD (orange), ADMM (green) and QN (cyan) solvers.}
...
approximation via an LBFGS update and adding a line search algorithm. Since the QN solver does not introduce any global linear momentum, it follows that the 
the reason for the PD solvers inability to preserve the linear momentum is the lack of one or both of these features of the QN solver. To find out which of 
...
We now switch our focus to the behavior of different solvers in the twisting beam experiment (see \autoref{fig:twisting-beam-setup}) using the strain material 
model described in \autoref{ss:material-models}. Recall that the position constraints required to simulate the twisting motion of the beam are incompatible 
with the tetrahedral strain constraints since there are no particle positions $\vecm{q}$ for which all constraint energies are minimized at the same time.

Again, we start our analysis by showing a representative plot of the objective function value over the 
solver iterations in \autoref{fig:strain-beam-twist-typical-objectives}. 

% TODO: Write general stuff here first!
% TODO: Paragraph headings!
...
\begin{figure}[h]
    ...
    \caption{Values of the objective function of the variational form of implicit Euler integration (\cref{eq:variational-implicit}) over the iterations of 
        a single solve (time step \SI{1e-2}{\second}) for a twisting beam using strain constraints with stiffness \num{1e9}.}
    \label{fig:strain-beam-twist-typical-objectives}
\end{figure}

During the untwisting beam experiments using the strain material model discussed above, each solver reduces 
...
and stiffness value. 
For the twisting beam experiment, this is not the case anymore: For various combinations of time step sizes and stiffness values,
the positions computed by XPBD after 1000 iterations yield a higher objective function value than the initial objective. We say that XPBD fails for 
such parameters. For each time step, the stiffness values for which XPBD fails are listed in 
...
more to XPBD failures in the twisting beam experiments for a variety of reasons. First, in contrast to the corresponding untwisting beam 
experiments, the XPBD constraint term is not only larger than the minimal constraint term achieved by PD-type solvers, but it is larger 
than the constraint term at the initial configuration. It is not obvious why the simplifications during the XPBD derivation would be less justified 
in the context of the twisting beam experiment. However, it is easy to see how mixing incompatible position and strain constraints could lead to more severe 
oscillations. Since there are no particle positions $\vecm{q}$ for which both the position constraints and the 
...
assume that the former scenario is more challenging for Gauss-Seidel-type solvers. Lastly, the severity of the oscillations is expected to increase with both 
time step size and stiffness values since both factors enable larger positions updates during individual constraint projections. This is in line with the 
...
also highlights one of its weaknesses: Due to the penalty incurred for moving particles away from their inertial particle positions and the fact that 
position constraints can only be modelled via stiff springs, exerting exact controll over a set of particle positions is challenging. Note that even the 
large stiffness value of \num{1e12} used for the position constraints in \autoref{fig:twisting-beam-xpbd-failure-geometry} is not sufficient to move particles 
...
iterations, the ADMM solver arrives at a lower objective function value than the initial objective after 60 iterations. Finally, after 80 iterations ADMM 
converges to a configuration where the objective function value is almost an order of magnitude lower than the minimal objective achieved by PD and QN.

\begin{figure}
    \begin{subfigure}{0.49\textwidth}
    \includegraphics[width=\textwidth, trim={0 5.0cm 0 2.5cm}, clip]{figures/twisted_beam_bad_state_admm.png}
    \subcaption{20 iterations}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    ...
    \end{subfigure}
    \caption{Geometries of a twisting beam with strain constraints (stiffness \num{1e10}) after 20 (left) and 1000 (right) ADMM iterations over time a time step 
        of \SI{1e-2}{\second}.}
