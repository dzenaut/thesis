...
law are reviewed in \cref{s:equations-of-motion}. Common approaches for numerical integration are introduced in \cref{s:numerical-integration}. 
...
\subsection{Numerical Integration of The Equations of Motion}\label{s:numerical-integration}
...
Thus, implicit Euler integration is more computationally expensive than both explicit and symplectic Euler integration. Despite the added 
complexity, implicit Euler integration is still only first-order accurate. However, implicit Euler integration remains a popular choice for 
physical simulation since it can be shown to be unconditionally stable \cite{chapra2005}. This allows dramatically increasing the size 
of the time steps. The additional cost of a single implicit Euler 
...
In \cref{ss:numerical-integration}, we have introduced various 

\begin{itemize}
    \item Describe the challenge: We have already outlined numerical integration schemes for the equations of motion, so what exactly
        is left to do?
    \begin{itemize}
        \item Stability for stiff systems. Sources of stiffness can be the material model itself (e.g. high values for Youngs modulus 
            order Poisson ratio) or constraints between bodies that are modeled via stiff forces (attaching bodies to each other / 
            fixing particle positions / implementing joints / contact handling). Usually, stability issues are due to the inherent 
            analytical properties of the underlying scheme for numerical integration (e.g. explicit Euler integration with large time steps 
            will eventually lead to instabilities, even 
            in the absence of numerical issues) or due to numerical issues (e.g. poor conditioning of the system matrix that arises 
            during implicit Euler integration). Especially in the real-time setting, stability is hard requirement. There is only one chance 
            to get the computation of the positions at the next time step right! 
        \item Avoid excessive damping. Excessive damping leads to visually implausible simulations and is particular noticable for stiff 
            systems. This issue mostly arises in the context of implicit Euler integration. The root cause is that the implicit Euler solver
            does not maintain physical invariants such as the preservation of energy and momentum. 
        \item Effiency, particularly for real-time simulations. Even a single solve of the linear system of equations from the implicit Euler
            integration can be infeasible if work from previous time steps cannot be reused. This is aggravated when the set of forces / 
            constraints changes from time step to time step. The more dense the sampling of the simulated bodies, the bigger this issue 
            becomes.
        ...
        \item How to model constraints? Issues above with stiff systems might suggest that handling via constraints might be favorable.
    \end{itemize}
\end{itemize}
