...
\noindent needs to be solved at every iteration as the Hessian matrix changes with the current iterate $\bm{x}_k$. For these reasons, while a single
...
\noindent where $\rho_{k-1} = 1 / (\bm{s}^T_{k-1}\bm{y}_{k-1})$. Alternatively, the update formula can be given in terms of $\bm{B}_k$ as
