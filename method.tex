...
\noindent shows that performing a Quasi-Newton step with Hessian approximation $\bm{B}_k = \bm{S}$ and step size $\alpha_k$ on the 
...
at the current iterate at all. On the other hand, Quasi-Newton methods are sensitive to the initial (inverse) Hessian 
approximation, which 
is nontrivial to choose. It stands to reason that due to its effectiveness in PD, $\bm{S}$ is a viable choice for the 
initial Hessian approximation of the BFGS or L-BFGS method, combining the advantages of the PD solver and line search 
methods from unconstrained optimization \cite{liu2017}. This is elaborated on in \cref{ss:qn-pd}
