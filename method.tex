...
\subsection{Simulation of Deformable Bodies}\label{ss:deformable-bodies}
...
positions $\vecm{q}$ of a potentially deformed body. In the following exposition, we lean heavily on the SIGGRAPH course by Sifakis et al.\ 
\cite{sifakis2012}, but restrict to the parts that are most relevant in the context of this thesis. 

We start by introducing the deformation function 
$\vecm{\phi} \colon \Sigma \mapsto \mathbb{R}^3$, where $\Sigma \subseteq \mathbb{R}^3$ is the volumetric domain by the undeformed occupied by the 
undeformed body. This domain $\Sigma$ is called the reference configuration. The deformation function maps every point in the reference configuration 
to its respective deformed position $\vecm{Q} = \vecm{\phi}(\vecm{q})$. Note that positions in the reference configuration are denoted using small 
letters while the deformed positions are referred to via capital letters. As an example, the deformation function given by 
...
\begin{itemize}
    \item In order to simulate deformable bodies numerical integration of the equations of motion, we need to be able to compute forces from the 
        positions of the deformed body. How? Refer to \cite{sifakis2012} and cover only the most important steps. Start with this. Otherwise, it is not 
        clear why the deformation gradient is relevant.
    \item Introduce energy density. Explain that the energy density can be expressed in terms of $\matm{F}$ since the energy density only needs to reflect
        the deformation behavior in an infitisemal neighborhood of the position of interest.
    \item Introduce the deformation gradient $\matm{F}$. If the deformation gradient is a rotation matrix, the body is only affected by a rigid body transform.
    \item Give one example to aid understanding. Introduce strain material, neohookean and simplified neohookean model.
    \item Show that the energy of a body can be computed by integrating the potential over the domain of the deformed body. 
    \item Show that the force acting on the body at a certain position can be computed by taking the derivative of the energy with respect to that position. 
    \item Show how the discretization works.
\end{itemize}
...
\subsection{Simulating Physically-Based Materials with PBD}\label{ss:pbd-physical}
\textcolor{red}{Write about the Bender paper here. Also mention somewhere that PBD technically implements hard constraints, but that they are only satisfied 
after a large number of iterations. At least position constraints can be truly hard by setting inverse massses to zero. Same thing is true for XPBD of course.}
...
and 60 iterations, whereas PD and XPBD converge after upwards of 260 and 1000 iterations, respectively. To quantify the progress that other solvers
...
Finally, each of the solvers converge to an inertial term that is significantly lower than the corresponding inertial term obtained with time step \SI{1e-2}{\second}
and stiffness \num{1e8}.
...
To gain further insights into how the large inertial term at the converged configuration from PD manifests itself, we repeat parts of the experiments in 
\autoref{fig:strain-beam-untwist-geometries} and take a closer look at the geometries that the solvers arrive at after 1000 iterations. 
