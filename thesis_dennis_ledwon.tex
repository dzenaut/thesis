...
numerical issues and poor convergence for many solvers. 
...
using the Taylor expansion. Positions can be eliminated can be expressed in terms of velocities and eliminated from the system.}
...
that the constraints are effectively linearized during one solve, e.g. \cite{}?}
...
properties for many solvers. The disadvantage of using hard constraints is that instabilities in the traverse direction of the constraints
occur when the tensile force with respect to particle masses is large \cite{tournier2015}.
